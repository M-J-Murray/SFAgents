...
class DataBins(object):
    ...
    def __init__(self, path, worker_count, frames_per_step):
        ...
        self.input_metadata = ((1, 61, 120, frames_per_step), "float32")
        self.move_metadata = ((1, 8), "uint8")
        self.attack_metadata = ((1, 9), "uint8")
        #self.reward_metadata = ((1,), "float64")
        ...
        #self.reward_bytes = len(np.zeros(*self.reward_metadata).tobytes())
    ...
    def insert_move_bin(self, worker_no, observation, move_action):
        ...
        #assert (len(reward_bytes) == self.reward_bytes), "Invalid reward inserted"
    ...
    def parse_move_data(self, data_bytes):
        ...
        #reward = np.frombuffer(data_bytes[self.input_bytes + self.move_bytes:], dtype=self.reward_metadata[1]).reshape(self.reward_metadata[0])
    ...
    def parse_attack_data(self, data_bytes):
        ...
        #reward = np.frombuffer(data_bytes[self.input_bytes + self.attack_bytes:], dtype=self.reward_metadata[1]).reshape(self.reward_metadata[0])
    ...
    def empty_move_bin(self, worker_no):
        ...
        line_bytes = self.input_bytes + self.move_bytes# + self.reward_bytes
    ...
    def empty_move_attack_bin(self, worker_no):
        ...
        line_bytes = self.input_bytes + self.move_bytes + self.attack_bytes# + self.reward_bytes
